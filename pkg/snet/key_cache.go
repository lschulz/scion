...
// All methods require external synchronization if used concurrently.
...
func (m *MockKeyProvider) GetASHostKey(
	ctx context.Context, validity time.Time, srcIA addr.IA,
) (drkey.Key, error) {
	return m.Key, nil
}

func (m *MockKeyProvider) GetHostSelfKey(
	ctx context.Context, validity time.Time, srcAddr addr.Addr,
) (drkey.Key, error) {
	return m.Key, nil
}

func (m *MockKeyProvider) GetHostHostKey(
	ctx context.Context, validity time.Time, srcAddr addr.Addr, dstAddr addr.Addr,
) (drkey.Key, error) {
	return m.Key, nil
}
...
// Refresh keys that will expire soon or already have expired assuming a global
// epoch duration of 'keyDuration'.
	...
	if c.cache != nil {
		for srcIA, keys := range c.cache {
			if len(keys) == 0 || keys[len(keys)-1].Epoch.NotAfter.After(t) {
				...
				if len(keys) > 0 && keys[len(keys)-1].Epoch.Covers(key.Epoch) {
					continue
				}
				c.cache[srcIA] = append(c.cache[srcIA], key)
			}
