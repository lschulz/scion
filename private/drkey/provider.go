...
import (
	...
	"github.com/scionproto/scion/private/drkey/grpc"
...
func NewProvider(localIA addr.IA, prefetchEntries int) (*Provider, error) {
	prefetchCache, err := arc.NewARC[addr.IA, struct{}](prefetchEntries)
	if err != nil {
		return nil, serrors.Wrap("creating Level1ARC cache", err)
	}
	provider := &Provider{
		localIA:       localIA,
		keyCache:      make(map[addr.IA]*cachedKey, prefetchEntries),
...
func (p *Provider) SetDialer(dialer libgrpc.Dialer) {
	p.dialer = dialer
}
...
// fetchLevel1Key gets a Level1 key from the CS. cacheLock must be held when
...
func (p *Provider) fetchLevel1Key(
	...
	go func() {
		...
		fetcher := grpc.Fetcher{
			Dialer: p.dialer,
		}
		key, err := fetcher.Level1Key(ctx, meta)
...
func (p *Provider) RefreshKeys(ctx context.Context) {
	fetcher := grpc.Fetcher{Dialer: p.dialer}
	validity := time.Now()
